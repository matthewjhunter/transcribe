...
- **`tstxt`** (default) -- `[2006-01-02T15:04:05] [SPEAKER_NN (X)]: text\n` when voice labeling is on, without the parenthetical when off. Timestamps are wall-clock by default (`--start-time auto`, derived from mtime minus duration) and fall back to `[HH:MM:SS]` offsets under `--start-time relative`. Recommended: timestamps let a human or LLM jump back to the source video for a specific moment in a multi-hour session, and wall-clock ones additionally merge against any other timestamped record of the same session (Roll20 chat logs, in the OSG case) with no offset to correlate; the `(M)`/`(F)`/`(?)` tag halves the candidate set when manually mapping clusters to players.
